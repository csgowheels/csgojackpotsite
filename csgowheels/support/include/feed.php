	...
	while( $row_trade = mysql_fetch_assoc($re_trade)){
	  ...
	  if($feedtype == '1')
	  {
		...
		$outputFeed .=  "<div class='row'><div class='col-md-2'><a href='".$personname['2']."'><img src='".$personname['1'].
		 				"' class='img-circle feed-profilepicture' /></a></div><div class='col-md-10'><p><span class='label label-primary betHead'>Bet</span> "
		 				.$personname['0']." deposited <span class='label label-default'>" . $row_trade['itemcount'] . " skins</span>with the value of <span class='label label-default'>$"
		 				.round($row_trade['ittotprice'],2)."</span>.</p></div>"; //*$row_updated['Amount']
	     $outputFeed .=  '</div><hr class="feed-divider"><br />';
	  }
	  else if($feedtype == '2')
	  {
	  	$query = "SELECT sum(tickettot)/(select sum(tickettot) from tradeitems where roundid='".
			$row_trade['roundid']."') as percentage FROM tradeitems INNER JOIN trade ON trade.TradeOfferId = tradeitems.tradeid INNER JOIN user 
			ON user.usteamid = trade.partnerId64 where roundid='".$row_trade['roundid']."' AND usteamid='".$row_trade['feed_user']
			."' GROUP BY tradeid order by trade.id asc;";
		$r = mysql_query($query) or die("DIE");
	  ...
	  {
	  	$outputFeed .=  "<div class='row newRound'><div class='col-md-2' style='width:1px;' ></div><div class='col-md-10' >
	  ...
	  {				
	  	$outputFeed .=  "<div class='row round'><div class='col-md-2' style='width:1%;'></div><div class='col-md-10'><span class='label label-primary'>Round #".$row_trade['roundid']." <br> Salt: ".$row_trade['itname'].
